...
"param!=value"：要求请求映射所匹配的请求可以不携带param请求参数但是携带的话，携带值一定不等于value
...
"header!=value"：要求请求映射所匹配的请求可以不携带header请求头信息若携带，携带值不一定等于value
...
<!--配置默认的servlet处理静态资源
        ...
        浏览器发送的请求回先被DispatcherServlet处理，处理不了，再交给DefaultServlet处理
    -->
 <mvc:default-servlet-handler/>
<!--开启mvc的注解驱动-->
<mvc:annotation-driven />
